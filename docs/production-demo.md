...
The local deployment acceptance runs thirteen checks (3 runtime/API contracts + 10 semantic sentinels) and ends with:
...
Authenticated public acceptance first proves an unauthenticated `/health` request returns HTTP `401`, then runs the same thirteen core checks. A complete public run therefore ends with:
...
Hard gates are intentionally distinct from the Search Showcase:

```text
Runtime/API contracts          = 3/3
English positive retrievals    = 4/4
Vietnamese positive retrievals = 4/4
No-answer intent protection    = 2/2
Semantic sentinels             = 10/10
Local acceptance               = 13/13
```

The exact sentinel strings are reused from committed canonical/Hard-v2/Hard-v3 evaluation corpora and are locked only after live qualification against the canonical 20,000-record snapshot.
...
- public PASS requires a real public acceptance log containing unauthenticated `401` and `PRODUCTION_DEMO_ACCEPTANCE_PASS=14`;
