...
Acceptance triển khai local thực hiện mười ba kiểm tra (3 contract runtime/API + 10 semantic sentinel) và kết thúc bằng:
...
Acceptance public có xác thực trước tiên chứng minh request `/health` không xác thực trả về HTTP `401`, sau đó chạy cùng mười ba kiểm tra cốt lõi. Do đó, một cuộc chạy công khai hoàn chỉnh sẽ kết thúc bằng:
...
Các hard gate được cố ý tách khỏi Search Showcase:

```text
Contract runtime/API              = 3/3
Positive retrieval tiếng Anh      = 4/4
Positive retrieval tiếng Việt     = 4/4
Bảo vệ no-answer / intent         = 2/2
Semantic sentinels                = 10/10
Local acceptance                  = 13/13
```

Các sentinel dùng lại chính xác query từ corpus canonical/Hard-v2/Hard-v3 đã commit và chỉ được khóa sau khi qualification live trên snapshot chuẩn 20.000 records.
...
- PASS công khai yêu cầu nhật ký chấp nhận công khai thực sự chứa `401` và `PRODUCTION_DEMO_ACCEPTANCE_PASS=14` chưa được xác thực;
