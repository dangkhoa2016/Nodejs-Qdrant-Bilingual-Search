...
test('lifecycle accepts the truthful qwen3 transformers cpu/float16 runtime contract', async () => {
  const temp = await mkdtemp(path.join(tmpdir(), 'production-demo-embedding-transformers-'))
  try {
    const fakeBin = await writeFakeEmbeddingCurl(temp, canonicalCpuEmbeddingRuntime({
      backend: 'transformers',
      device: 'cpu',
    ...
    const env = cpuEmbeddingLifecycleEnv(temp, fakeBin)
    env.EMBEDDING_DTYPE = 'float16'
    const command = `source ${JSON.stringify(lifecycle)}; start_embedding`
    const { stdout, stderr } = await execFileAsync('bash', ['-c', command], {
      cwd: projectRoot,
      env
    ...
    assert.match(stdout, /Embedding service\s+READY \(external\/reused\)/)
    assert.doesNotMatch(stderr, /runtime contract mismatch|refusing reuse/)
    await assert.rejects(readFile(path.join(temp, 'runtime', 'embedding.pid'), 'utf8'), { code: 'ENOENT' })
  } finally {
    await rm(temp, { recursive: true, force: true })
  }
