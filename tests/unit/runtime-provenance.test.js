...
test('semanticIndexCompatibilityRuntime accepts a transformers CPU executor with identical semantic identity', () => {
  const projected = semanticIndexCompatibilityRuntime(transformersRuntime)
  assert.equal(projected.backend, 'transformers')
  assert.equal(projected.implementation, 'python-fastapi')
  assert.equal(projected.semantic, true)
  assert.equal(projected.semanticAuditOnly, true)
  assert.equal(projected.profile, 'qwen3')
  assert.equal(projected.query_strategy, 'prompt')
  assert.equal(projected.query_instruction_id, 'geo-retrieval-v1:abc')
  assert.equal(projected.document_strategy, 'raw')
...
test('semanticIndexCompatibilityRuntime treats execution provenance as non-authoritative across backends', () => {
  const projected = semanticIndexCompatibilityRuntime(transformersRuntime)
  assert.equal('accelerator' in projected, false)
  assert.equal('device' in projected, false)
  assert.equal('dtype' in projected, false)
...
test('semanticIndexCompatibilityRuntime still rejects a non-semantic transformers executor', () => {
  assert.throws(() => {
    semanticIndexCompatibilityRuntime({ ...transformersRuntime, semantic: false })
  }, /verified semantic embedding backend is required/i)
  assert.throws(() => {
    semanticIndexCompatibilityRuntime({ ...transformersRuntime, backend: 'mock', implementation: 'mock-server' })
  }, /verified semantic embedding backend is required/i)
})
