...
test('package exposes one-command real production consistency acceptance', async () => {
  ...
  assert.equal(pkg.scripts['acceptance:v21-production-consistency:run'], 'NODE_ENV=development node scripts/acceptance/v21-production-consistency.mjs')
  assert.equal(pkg.scripts['acceptance:v21-production-consistency'], 'bash scripts/acceptance/v21-production-consistency.sh')
  await access(repoFile('scripts/acceptance/v21-production-consistency.mjs'))
