...
test('verifyEmbeddingRuntime semantic audit accepts a transformers CPU executor against a GPU-seeded sentence-transformers payload', async () => {
  const requests = []
  const payload = {
    embedding_backend: 'sentence-transformers',
    embedding_implementation: 'python-fastapi',
    ...
    embedding_profile: 'qwen3',
    embedding_query_strategy: 'prompt',
    embedding_query_instruction_id: 'geo-retrieval-v1:d014d3ec6df87e49',
    embedding_document_strategy: 'raw',
    embedding_model: 'Qwen/Qwen3-Embedding-4B',
    embedding_text_version: 'v2.1'
  }
  ...
  const service = new QdrantService({ connection: fakeConnection(client), collection: 'c', dimension: 2560 })
  const runtime = {
    backend: 'transformers', implementation: 'python-fastapi', semantic: true,
  ...
  assert.equal(result.matchingCount, 1)
  assert.equal(requests[0].with_payload.includes('embedding_profile'), true)
  assert.equal(requests[0].with_payload.includes('embedding_query_strategy'), true)
  assert.equal(requests[0].with_payload.includes('embedding_query_instruction_id'), true)
  assert.equal(requests[0].with_payload.includes('embedding_document_strategy'), true)
})

test('verifyEmbeddingRuntime semantic audit still rejects a semantic identity mismatch (profile)', async () => {
  const client = {
    getCollection: async () => ({ config: {} }),
    scroll: async () => ({
      points: [{
        ...
        payload: {
          embedding_backend: 'sentence-transformers', embedding_implementation: 'python-fastapi', embedding_semantic: true,
          embedding_profile: 'e5', embedding_query_strategy: 'prefix', embedding_document_strategy: 'prefix'
        }
      }],
      next_page_offset: null
    })
  ...
  await assert.rejects(() => service.verifyEmbeddingRuntime({
    ...
    runtime: {
      backend: 'transformers', implementation: 'python-fastapi', semantic: true,
      profile: 'qwen3', query_strategy: 'prompt', document_strategy: 'raw'
    },
    semanticAuditOnly: true
  }), /embedding runtime provenance verification failed/)
})

test('verifyEmbeddingRuntime semantic audit still requires a verified semantic runtime', async () => {
  const client = {
    getCollection: async () => ({ config: {} }),
  ...
  await assert.rejects(() => service.verifyEmbeddingRuntime({
    expectedPoints: 1,
    runtime: { backend: 'transformers', implementation: 'python-fastapi', semantic: false },
    semanticAuditOnly: true
  }), /verified semantic embedding backend is required/i)
  await assert.rejects(() => service.verifyEmbeddingRuntime({
    expectedPoints: 1,
    runtime: { backend: 'mock', implementation: 'mock-server', semantic: true },
    semanticAuditOnly: true
  }), /verified semantic embedding backend is required/i)
})
