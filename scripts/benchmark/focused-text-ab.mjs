...
const outputPath = resolve(process.env.FOCUSED_AB_OUTPUT ?? 'reports/qwen3-4b-text-v1-v2-focused-ab.json')
const textsOutputPath = resolve(process.env.FOCUSED_AB_TEXTS_OUTPUT ?? 'reports/qwen3-4b-text-v1-v2-focused-candidate-texts.json')
const manifestOutputPath = resolve(process.env.FOCUSED_AB_MANIFEST_OUTPUT ?? 'reports/qwen3-4b-text-v1-v2-focused-candidate-manifest.json')
...
const result = await runFocusedTextAbExperiment({
  cases,
  hardReport,
  entities,
...
const controlledVariables = {
  ...
  similarity: 'cosine',
  resultLimit: 5,
  onlyChangedVariable: 'document embedding text: v1 vs v2'
}
...
console.log(JSON.stringify({
  ...
  note: 'No-diacritics cases are reported separately and must not be used alone to infer document-text v2 causality.'
}, null, 2))
