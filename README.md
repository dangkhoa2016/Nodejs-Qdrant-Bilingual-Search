...
These smoke sentinels are a **compact, deterministic end-to-end regression gate**, not a quality benchmark for the full 20K corpus. They verify that the accepted `query → embedding → Qdrant → search policy → API response` path continues to produce known positive and negative behavior after runtime, snapshot, or deployment changes. Broader retrieval quality is evaluated separately by the committed benchmark suites in [benchmarks/README.md](benchmarks/README.md).
